...
export class GitBranchService {
    ...
    private async handleExistingBranch(branchName: string): Promise<string | null> {
        ...
        const choice = await vscode.window.showQuickPick(
            ...
            {
                placeHolder: this.strings.branch_exists_message.replace('{0}', branchName),
            }
        );
            ...
            return await new Promise((resolve) => {
                ...
                disposables.push(
                    vscode.workspace.onDidSaveTextDocument(async (doc) => {
                        if (!isPromptShowing && files.some(f => doc.uri.fsPath.endsWith(f))) {
                            isPromptShowing = true;
                            
                            const choice = await vscode.window.showInformationMessage(
                                this.strings.conflict_resolution_prompt,  // Different message for completion
                                { modal: true, detail: this.strings.conflict_resolution_detail },
    ...
    private async openConflictedFiles(files: string[]): Promise<void> {
        ...
        for (const file of files) {
            try {
                ...
                console.error(
                    this.strings.error_unknown.replace('{0}', file),
                    error
                );
            }
        }
