...
(function () {
    ...
    switch (message.type) {
      ...
      case "prUrlValidation":
        if (!message.payload.isValid && formElements.prUrlError) {
          ...
          formElements.prUrlError.textContent =
            state.strings.error_invalid_pr_url;
  ...
  function initializeFormElements() {
    formElements = {
      ...
      newRepoNameInput: document.getElementById(
        "newRepoName",
      ) as HTMLInputElement,
      cherryPickInput: document.getElementById(
        "cherryPickCommit",
      ) as HTMLInputElement,
      newRepoNameLabel: document.querySelector(
        'label[for="newRepoName"]',
      ) as HTMLLabelElement,
  ...
  function updatePlaceholders() {
    if (formElements.prUrlInput) {
      formElements.prUrlInput.placeholder =
        state.strings.pr_url_field_placeholder || "";
    }
  ...
  function validateFormState(): boolean {
    const hasRepo =
      formElements.repoNameSelect?.value.trim() ||
      formElements.newRepoNameInput?.value.trim();
    ...
    if (hasCherryPick) {
      ...
      if (!isValidCommit && formElements.commitError) {
        formElements.commitError.textContent =
          state.strings.invalid_commit_format;
    ...
    const isValid = Boolean(
      hasRepo && hasVersions && hasCherryPick && isValidCommit && hasPrUrl,
    );
  ...
  function validateForm(data: BackportFormData): {
    isValid: boolean;
    error?: string;
  } {
    if (!data.versions || !data.cherryPickCommit || !data.prUrl) {
      return {
        isValid: false,
        error: strings.validation_versions_cherry_pick_required,
      };
    }
  ...
  function addVersion(version: string) {
    ...
    let versions = formElements.versionsInput.value
      .split(",")
      .map((v) => v.trim())
      .filter((v) => v);
  ...
  function showValidationError(message: string) {
    const versionsInput = document.getElementById(
      "versions",
    ) as HTMLInputElement;
